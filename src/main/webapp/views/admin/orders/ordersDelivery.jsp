						...
						<tbody>
							...
							<c:forEach var="item" items="${ orders }">
								<tr>
									...
									<td style=""><form id="${ item.id }" method="post" action="admin-orders?type=delivered&id=${ item.id }&userid=${ item.userId }">
...
">complete delivery</button></form></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
	...
	<div style="margin-top: 50px;">
		...
		<ul class="pagination modal-5">
			...
			<c:forEach begin="1" end="${ endP }" var="i">
				<li><a class="page"
					href="<c:url value="/admin-orders?type=delivery&index=${ i }" />">${ i }</a></li>
			</c:forEach>
	...
	<script type="text/javascript">
		...
		if(${inext} <= ${ endP }){
			...
			next.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-delivery?type=wait&index=${inext}" />";
			})
		}
		...
		if(${page} > 0) {
			...
			prev.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-delivery?type=wait&index=${page}" />";
			})
		}
