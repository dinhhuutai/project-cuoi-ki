		...
		<div class="row column_title">
			<div class="col-md-12">
				<div class="page_title">
					<h2>List orders wait for confirm</h2>
				</div>
			</div>
		</div>
	</div>
						...
						<tbody>
							...
							<c:forEach var="item" items="${ orders }">
								<tr>
									...
									<td style=""><form id="${ item.id }" method="post" action="admin-orders?type=confirm&id=${ item.id }&userid=${ item.userId }">
										<button type="submit" value="${ item.id }" class="cancel-orders" style="
    margin-right: 12px;
    padding: 2px 5px;
    background-color: #1faf02;
    color: #fff;
    border-radius: 4px;border: none;cursor: pointer;
">Confirm</button></form>
									<form id="${ item.id }" method="post" action="admin-orders?type=cancel&id=${ item.id }&userid=${ item.userId }">
										<button type="submit" value="${ item.id }" class="cancel-orders" style="
	...
	<div style="margin-top: 50px;">
		...
		<ul class="pagination modal-5">
			...
			<c:forEach begin="1" end="${ endP }" var="i">
				<li><a class="page"
					href="<c:url value="/admin-orders?type=wait&index=${ i }" />">${ i }</a></li>
			</c:forEach>
	...
	<script type="text/javascript">
		...
		if(${inext} <= ${ endP }){
			...
			next.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-orders?type=wait&index=${inext}" />";
			})
		}
		...
		if(${page} > 0) {
			...
			prev.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-orders?type=wait&index=${page}" />";
			})
		}
