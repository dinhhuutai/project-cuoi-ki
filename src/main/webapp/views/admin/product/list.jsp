	...
	<div style="margin-top: 50px;">
		...
		<ul class="pagination modal-5">
			...
			<c:forEach begin="1" end="${ endP }" var="i">
				<li><a class="page"
					href="<c:url value="/admin-product?type=list&index=${ i }" />">${ i }</a></li>
			</c:forEach>
	...
	<script type="text/javascript">
		...
		if(${inext} <= ${ endP }){
			...
			next.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-product?type=list&index=${inext}" />";
			})
		}
		...
		if(${page} > 0) {
			...
			prev.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-product?type=list&index=${page}" />";
			})
		}
