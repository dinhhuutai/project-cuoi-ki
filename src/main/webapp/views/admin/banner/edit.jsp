...
<body>
			...
			<div class="col-md-12">
				<div class="page_title" style="display: flex;">
					<a href='<c:url value="/admin-banner?type=list"/>'>
						<h4 style="font-size: 14px; opacity: 0.5;">List Banner</h4>
					</a>
	...
	<form id="formSubmit" class="row g-3" method="post"
		<c:if test="${empty model.id}">
		action="admin-upload-img-banner?type=edit&message=insert_success"
		</c:if>
		<c:if test="${not empty model.id}">
		action="admin-upload-img-banner?type=list&index=1&message=update_success"
		</c:if>
    ...
    function addBanner(data) {
        $.ajax({
            ...
            error: function(error) {
                window.location.href = "${BannerURL}?type=edit&message=error_system";
            },
        });
    }
