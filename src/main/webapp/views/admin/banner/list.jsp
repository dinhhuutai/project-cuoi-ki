...
<c:url var="BannerURL" value="/admin-banner" />
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8" />
<title>List Slie</title>
</head>
<body>
			...
			<div class="pull-right tableTools-container">
				<div class="dt-buttons btn-overlap btn-group"
					...
					<a flag="info"
						...
						href='<c:url value="/admin-banner?type=edit"/>'
			...
			<div class="col-12">
				<div style="border-radius: 8px; overflow: hidden;">
					<table class="table table-striped"
						...
						<thead style="background-color: #04a4ec; color: #fff;">
							<tr>
								<th scope="col">
    ...
    -webkit-line-clamp: 1;
">${ item.info }</td>
									<td><c:url var="editURL" value="/admin-banner">
	...
	<div style="margin-top: 50px;">
		...
		<ul class="pagination modal-5">
			...
			<c:forEach begin="1" end="${ endP }" var="i">
				<li><a class="page"
					href="<c:url value="/admin-slide?type=list&index=${ i }" />">${ i }</a></li>
			</c:forEach>
	...
	<script type="text/javascript">
		...
		if(${inext} <= ${ endP }){
			...
			next.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-banner?type=list&index=${inext}" />";
			})
		}
		...
		if(${page} > 0) {
			...
			prev.addEventListener('click', function() {
				window.location.href = "<c:url value="/admin-banner?type=list&index=${page}" />";
			})
		}
		...
		function deleteSlide(data) {
	        $.ajax({
	            ...
	            success: function(result) {
	                window.location.href = "${BannerURL}?type=list&index=${ pagecurr }&message=delete_success";
	            },
	            error: function(error) {
	                window.location.href = "${BannerURL}?type=list&index=${ pagecurr }&message=error_system";
	            },
	        });
		}
