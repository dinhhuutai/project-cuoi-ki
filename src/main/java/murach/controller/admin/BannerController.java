...
public class BannerController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		BannerModel model = FormUtil.toModel(BannerModel.class, req);
		...
		if (model.getType().equals(SystemConstant.LIST)) {
			...
			int count = bannerService.getTotalItem();
			...
			req.setAttribute("banner", bannerService.findAll(index, itemInPage));
			...
			if(model.getId() != null) {
				model = bannerService.findOne(model.getId());
